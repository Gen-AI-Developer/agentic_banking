...
class MyCustomAgentHook(AgentHooks):
    async def on_start(self, context,agent) -> None:
        print(f"AH:-> Agent LifeCycle on_start with {agent.name} is started")

    async def on_end(self, context,agent,output) -> None:
        print(f"AH:->Agent LifeCycle on_end with {agent.name} has completed")

class MyCustomRunHook(RunHooks):
    async def on_agent_start(self,context,agent):
        print(f"RH:-> Run LifeCycle is started with on_agent_start agent: {agent.name}")

    async def on_agent_end(self,context, agent, output):
        print(f"RH:-> Run LifeCycle is ended with on_agent_end and has completed with agent: {agent.name}")
...
def main():
    ...
    result = Runner.run_sync(triage_agent, "what is agent as per Open AI Agents SDK?",hooks=MyCustomRunHook())
